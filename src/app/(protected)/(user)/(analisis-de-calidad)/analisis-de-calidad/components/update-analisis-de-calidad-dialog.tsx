...
import { Calendar, MapPin, Package, Leaf, Palette, AlertTriangle, CheckCircle2, Ruler, CheckCircle, ParkingMeter, Pencil } from "lucide-react"
import { AnalisisDeCalidad } from "@/types/analisis-de-calidad/analisis-de-calidad.type"
import { updateAnalisisDeCalidadAction } from "@/actions/analisis-de-calidad/update-analisis-de-calidad.action"

export function UpdateAnalisisDeCalidadDialog({
  analisisDeCalidad,
  etapas,
  cuarteles,
  unidades,
  ...
  const form = useForm<AnalisisDeCalidadSchemaType>({
    ...
    defaultValues: {
      fecha: analisisDeCalidad.fecha
        ? new Date(analisisDeCalidad.fecha).toISOString().split("T")[0]
        : new Date().toISOString().split("T")[0],
      etapaId: analisisDeCalidad.etapaInspeccion?.id ?? "",
      cuartelId: analisisDeCalidad.cuartel?.id ?? "",
      unidadInspeccion: {
        id: analisisDeCalidad.unidadInspeccion?.id ?? "",
        cantidad: analisisDeCalidad.unidadesInspeccion?.length ?? 1,
      },
      universoMuestra: analisisDeCalidad.universoMuestra ?? undefined,
      inspecciones:
        analisisDeCalidad.unidadesInspeccion?.map((u) => ({
          indice: u.indice,
          especieId: u.especie?.id ?? "",
          variedadId: u.variedad?.id ?? "",
          calibres:
            u.calibres?.map((c) => ({
              calibre: c.calibre,
              cantidad: c.cantidad,
          ...
          colores:
            u.colores?.map((col) => ({
              ...
              porcentaje: col.porcentaje ?? 0,
            })) ?? [],
          controlesCalidad:
            u.controlesCalidad?.map((ctrl) => ({
              defectos:
                ctrl.defectosAsignados?.map((a) => ({
                  id: a.defecto?.id ?? "",
          ...
          temperaturaBins: u.temperaturaBins ?? 0,
          brix: u.brix ?? 0,
        })) ?? [],
    },
  })
  ...
  useEffect(() => {
    console.log("⚙️ Errores:", form.formState.errors)
  }, [form.formState.errors])

  const onSubmit = (values: AnalisisDeCalidadSchemaType) => {
    startTransition(async () => {
      const res = await updateAnalisisDeCalidadAction(analisisDeCalidad.id, values)

      if (res?.error) {
        toast.error(res.error || "❌ Error al actualizar el análisis")
        return
      }
      ...
      toast.success(
        <div className="flex items-center gap-2">
          ...
          <span>Análisis actualizado correctamente</span>
        </div>
      )
      ...
      <Button
        ...
        onClick={() => {
          ...
          form.setValue(path, [
            ...current,
            {
                fecha: form.getValues("fecha") || new Date().toISOString().split("T")[0],
                [placeholders[0].toLowerCase()]: "",
                cantidad: 0,
              }
